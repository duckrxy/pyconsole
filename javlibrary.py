...
class JavLibrary(object):
    ...
    def SearchNumber(self, number):
        ...
        r = self.session.get(self.__javSearchUrl__,
                             headers=self.__header__, params=query)
        title = self.GetTitle(r.text, '<title>(.*) - JAVLibrary<\/title>')
        if (title == ""):
            raise Exception('Not Found')
        if (title.find('识别码搜寻结果') != -1):
            redirectedUrlReg = str.format(
                "<a href=\"(.*)\" title=\"{0}.*\">", upperNumber)
            redirectedPageUrl = self.__javHomeUrl__ + \
                self.GetTitle(r.text, redirectedUrlReg)
            redirectedPage = self.session.get(
                redirectedPageUrl, headers=self.__header__)
            redirectedTitle = self.GetTitle(
                redirectedPage.text, '<title>(.*) - JAVLibrary<\/title>')
            if (redirectedTitle != ""):
                actress = self.GetActress(redirectedPage.text)
                return {'title': redirectedTitle, 'actress': actress}
            else:
                raise Exception('Not Found')
        actress = self.GetActress(r.text)
        return {'title': title, 'actress': actress}
